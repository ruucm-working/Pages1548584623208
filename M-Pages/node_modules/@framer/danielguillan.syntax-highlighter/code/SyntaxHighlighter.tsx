...
// 4. Hit Preview (⌘ + P) to see the result live!`;
  ...
  _getCodeSnippetFromChildren(children) {
    return React.Children.map(children, child => {
      ...
      if (
        ...
        return child.props.contentState.blocks.map(b => b.text).join("\n");
        // Canvas
      } else if (
        child.props.contentState &&
        child.props.contentState.blockMap
      ) {
        return child.props.contentState.blockMap.map(b => b.text).join("\n");
        // Nested children
      } else if (child.props && React.Children.count(child.props.children)) {
  ...
  _parseFocusedLines() {
    ...
    const lines = ranges.reduce((prev, range) => {
      ...
      const rangeLines = Array.apply(null, Array(endLine - startLine + 1)).map(
        (item, idx) => idx + startLine
      );
    ...
    return (
      <div style={{ width: width, height: height }}>
        <SyntaxHighlighter
          ...
          style={
            require(`react-syntax-highlighter/dist/styles/hljs/${
              themes[style]
            }`).default
          }
        >
          {codeSnippet.length ? codeSnippet.join() : this.instructionsSnippet}
        </SyntaxHighlighter>
      </div>
    );
  }
